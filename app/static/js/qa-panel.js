...
function makeDraggable(element, handle) {
  ...
  let posX = parseInt(style.left) || 20;
  let posY = parseInt(style.top) || window.innerHeight - element.offsetHeight - 20;

  // Set initial position if not already set
  if (style.bottom === 'auto' || !style.bottom) {
  ...
  function dragStart(e) {
    ...
    // Add listeners to track movement and release
    document.addEventListener('mousemove', dragMove);
    document.addEventListener('mouseup', dragEnd);
    
    // Add dragging class for style changes during drag
  ...
  function dragEnd(e) {
    ...
    // Ensure panel stays visible within viewport
  ...
  // Attach mouse down event to handle
  if (handle) {
