...
function formatQAResponse(answer, functions) {
  ...
  if (functions && functions.length > 0) {
    functions.forEach((func, index) => {
      ...
      const funcRegex = new RegExp(`(${escapeRegExp(funcName)}|${escapeRegExp(fullName)})`, 'g');
      formatted = formatted.replace(funcRegex, `<a class="function-link" data-function-id="${func.id}">$1</a>`);
    });
  }
...
function saveQAMessages() {
  ...
  const messages = [];
  const messageElements = qaMessages.querySelectorAll('.chat-message:not(.loading)');
